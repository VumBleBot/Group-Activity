...
# 파이썬 설치
sudo apt-get -y install libncursesw5-dev libgdbm-dev libc6-dev zlib1g-dev libsqlite3-dev tk-dev
...
cd $HOME/Python-3.6.9 && sudo make install
yes | sudo rm -R $HOME/Python-3.6.9
...
sudo make install
yes | 
cd .. && sudo rm -R mecab-0.996-ko-0.9.2.tar.gz
# mecab_ko_dict 설치 
...
sudo make install
yes | 
cd .. && sudo rm -R mecab-ko-dic-2.1.1-20180720.tar.gz
# mecab pytho 설치 
